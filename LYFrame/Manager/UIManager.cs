...
namespace LYFrame
{
    public class UIManager : ManagerBase
    {
        ...
        public Dictionary<string, MyCanvas> canvasDic = new Dictionary<string, MyCanvas>();

        private MyCanvas currentCanvas;

        public MyCanvas CurrentCanvas
        {
            get
            {
            ...
            {
                currentCanvas = value;
                AlterCanvas(currentCanvas);
                UIMaskMgr.GetInstance().InitMask(currentCanvas);
            }
        }
        ...
        //public BasePart GetPart(string componentCode)
        //{
        //    BasePart result;
        //    dicPart.TryGetValue(componentCode, out result);
        //    return result;
        //}

        //public void RegistPart(string id, BasePart part)
        //{
        //    if (!dicPart.ContainsKey(id))
        //    {
        //        dicPart.Add(id, part);
        //    }
        //}
        ...
        //public void ResAllPartData()
        //{
        //    foreach (KeyValuePair<string, BasePart> kvp in dicPart)
        //    {
        //        kvp.Value.ResetState(null);
        //    }
        //}
        ...
        public bool FormActive(string uiFormName)
        {
            BaseUIForm baseUIForm = null;
            if (string.IsNullOrEmpty(uiFormName)) return false;
            baseUIForm = LoadFormsToAllUIFormsCatch(uiFormName);
            return baseUIForm.gameObject.activeInHierarchy;
        ...
        {
            int count = _DicCurrentShowUIForms.Count;
            int staCount = _StaCurrentUIForms.Count;
            if (count == 0 && staCount == 0) return;
            string[] formsName = new string[count];
            _DicCurrentShowUIForms.Keys.CopyTo(formsName, 0);
            for (int i = 0; i < count; i++)
            {
                string formName = formsName[i];
                Debug.Log(formName);
                ExitUIForms(formName);
            }
            if (staCount == 0) return;
        ...
        private BaseUIForm LoadUIForm(string uiFormName)
        {
            ...
            Debug.Log(_TraCanvasTransfrom);
            Debug.Log(goCloneUIPrefabs);
