...
namespace LYFrame
{
    public class Timer
    {
        ...
        public static void EndTime(string flag)
        {
            Timer timer = GetTimer(flag);
            if (null != timer)
            {
                if (null != timer.EndEvent) timer.EndEvent.Invoke();
                DelTimer(flag);
            }
            else
            {
        ...
        {
            timers.ForEach((v) => { v.Pause();});
        }

        /// <summary>
        /// 恢复暂停的所有计时触发器
        /// </summary>
        public static void ResumAll()
        {
            timers.ForEach((v) => { v.Resum();});
        }
